	...
	*************************/
	...
	$('#slide-3 img').each(function(index, element) {
		...
		myImg.load(function(e) {
			img_loaded += 1;;
			if ( img_loaded == $('#ex-slide-3 img').length ) {*/
				$(function() {
					var pause = 10;
...
$(function() {
	...
	$(window).resize(function() {
		delay(function() {
				var gallery_images = $('#ex-slide-3 img');
				...
				var gallery_width = $('#ex-slide-3 img').width() * $('#ex-slide-3 img').length;
				gallery_width /= 2;
				if ( $('#ex-slide-3 img').length % 2 != 0 ) {
					gallery_width += $('#ex-slide-3 img').width();
				}
				
				$('#ex-slide-3 .row').css('width', gallery_width );
				
				var left_pos = $('#ex-slide-3 .row').width() - $('body').width();
				left_pos /= -2;
				
				$('#ex-slide-3 .row').css('left', left_pos);
...
jQuery(document).ready(function ($) {
	//Cache some variables
	var images = $('#ex-slide-3 a');
	
	images.hover(
		function(e) {
			var asta = $(this).find('img');
			$('#ex-slide-3 img').not( asta ).stop(false, false).animate(
				{
					opacity: .5
				},
				'fast',
		...
		function(e) {
			$('#ex-slide-3 img').stop(false, false).animate(
				{
					opacity: 1
				},
				'fast',
...
jQuery(document).ready(function ($) {
	...
	arrows.click(function(e) {
		...
		switch( $(this).attr('id') ) {
			...
			case 'arrow-left':
				//PORTFOLIO
				//offset_left = $('#ex-slide-3 .row').offset().left + 452;
				if ( offset_left > 0 ) {
					offset_left = '0px';
				}
				break;
			case 'arrow-right':
				// PORTFOLIO
				//offset_left = $('#ex-slide-3 .row').offset().left - 452;
				if ( offset_left < $('body').width() - $('#ex-slide-3 .row').width() ) {
					offset_left = $('body').width() - $('#ex-slide-3 .row').width();
				}
				break;
		}
		...
		if ( offset_left != false ) {
			//PORTFOLIO 
			/*
			if ( $('#ex-slide-3 .row').width() != $('body').width() ) {
				$('#ex-slide-3 .row').stop(false, false).animate({
					left: offset_left
				}, 1500, 'easeInOutQuart');
			} */
		}
